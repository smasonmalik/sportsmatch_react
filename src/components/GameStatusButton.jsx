...
class ConfirmGameButton extends React.Component {
  ...
  render() {
    if ((this.state.status === "confirmed") || (this.props.organiser_id === parseInt(localStorage.getItem('user_id')) && this.state.status === "pending")) {
      return (
        <div>
          <button id="cancel-game" className="btn btn-danger" value="cancelled" onClick={this.handleClick}>Cancel Game</button>
        </div>
      )
    } else if (this.state.status === "pending" && this.props.opponent_id === parseInt(localStorage.getItem('user_id'))) {
      return (
        <div>
          <button id="confirm-game" className="btn btn-success" value="confirmed" onClick={this.handleClick}>Confirm Game</button>
          <button id="decline-game" className="btn btn-danger" value="declined" onClick={this.handleClick}>Decline Game</button>
        </div>
      )
    }
    else {
